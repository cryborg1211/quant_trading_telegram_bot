...
# retrain (heavy — full stack, ~10+ min)
...
src/data/db_engine.py                        DuckDB singleton + schema migrations (macro_daily, etc.)
src/data/crawlers.py                         StockCrawler (vnstock) + MacroCrawler (yfinance+SBV)
...
**DuckDB:** `data/quant_v6_core.duckdb` (gitignored). Tables (when populated):

| Table | Status | Notes |
|---|---|---|
| `stock_ohlcv` | ✅ 851,787 rows (355 tickers, 2014-04→2026-05) | Bulk-loaded from `data/ohlcv_*.parquet` |
| `macro_daily` | ✅ 3,224 rows (2014→2026) | yfinance for S&P500/DXY/USD-VND; **`interbank_on_rate`/`vnibor` are NULL** (SBV+TradingEconomics DNS-blocked from VN ISPs — TD-25) |
...
**Parquet:** `data/ohlcv_<TICKER>.parquet` (355 files, **gitignored**) ·
`data/alpha360_features.parquet` (regenerated by `build_alpha360`,
~366 cols × 850k rows, **gitignored**).

**Crawl pipeline:** `MacroCrawler.fetch_macro()` → yfinance + SBV/TE
(VN-DNS sensitive); `SentimentCrawler` → GNews per-ticker + Vietnamese
RSS → `_score_item` → `hist_sentiment_llm_labeled`. The backfill script
re-uses both via the existing infra (no divergent schemas).
...
| TD-25 | SBV + TradingEconomics interbank rate scrapers DNS-blocked from VN ISPs. `macro_daily.interbank_on_rate` is NULL. | Open |
...
| Add a 5d feature | Inject into `Alpha360Generator` → regenerate parquet (`main.py --task build_alpha360`) → re-train (`train_stacking.py`); this is the heavy path (~10+ min) |
