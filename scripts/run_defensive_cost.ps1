...
$log = "logs\defensive_cost_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
Add-Content $log "===== Defensive layer cost (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_defensive_layer_cost.py >> `"$log`" 2>&1"
