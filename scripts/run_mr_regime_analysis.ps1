...
$log = "logs\mr_regime_analysis_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"

Add-Content $log "===== MR regime-conditioning analysis (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_mr_regime_conditioning.py >> `"$log`" 2>&1"
