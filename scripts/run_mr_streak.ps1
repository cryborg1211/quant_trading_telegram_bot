...
$log = "logs\mr_streak_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
Add-Content $log "===== MR + streak features (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_mr_streak_features.py >> `"$log`" 2>&1"
