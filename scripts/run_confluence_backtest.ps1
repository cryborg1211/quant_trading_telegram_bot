...
$log = "logs\confluence_backtest_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"

Add-Content $log "===== Confluence backtest analysis (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_confluence_backtest.py >> `"$log`" 2>&1"
