...
$log = "logs\dsr_calibration_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"

Add-Content $log "===== DSR/PBO calibration diagnostic (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_dsr_calibration.py >> `"$log`" 2>&1"
