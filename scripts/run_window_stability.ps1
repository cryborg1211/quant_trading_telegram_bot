...
$log = "logs\window_stability_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
Add-Content $log "===== Window stability (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_window_stability.py >> `"$log`" 2>&1"
