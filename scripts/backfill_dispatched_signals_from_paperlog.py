...
table currently has ZERO rows ever written â€” the live ``record_dispatch`` path
...
âš ï¸ THIS IS A PROXY, NOT A VERIFIED DISPATCH RECORD.
...
available approximation of "the engine suggested this ticker" â€” but it is not a
...
today's EOD report does NOT wrongly show weeks of backlog under "ÄÃƒ ÄÃ“NG HÃ”M NAY":
...
All backfilled rows carry ``weight = 0.0`` (paper-tracking only â€” these were never
...
Usage (dry-run by default â€” prints what WOULD be inserted, writes nothing):
...
_DAILY_SOURCE = "daily"
_BUY = 2  # decision code: argmax over [DOWN, SIDE, UP] == index 2 (UP) â†’ BUY

# Horizon â†’ hold_days conventions (see module docstring).
...
PaperlogRow = namedtuple(
    "PaperlogRow",
    "log_date ticker source decision_primary p_down_secondary p_side_secondary p_up_secondary entry_close",
)
...
def decision_from_probs(
    ...
    """Argmax over ``[DOWN, SIDE, UP]`` â†’ decision code (0/1/2), or None if any
...
def _resolve_status(
    ...
    when the row must be skipped (missing/non-positive t0 â†’ no usable anchor).
    ...
    Never raises â€” a price-lookup miss degrades to a skip (None), never a crash.
    """
    try:
        ...
        return "OPEN", None
    except Exception as exc:  # noqa: BLE001 â€” best-effort backfill, never crash on one row
...
def build_backfill_plan(
    ...
    """Pure planning core: paperlog rows â†’ reconstructed ledger rows + counts.

    ``existing_keys`` is the set of ``(ticker, dispatch_date, horizon)`` tuples
    already present in ``dispatched_signals`` â€” candidates matching an existing
    ...
    Non-``daily`` source rows (e.g. ``source='verify'``) are excluded â€” those are
...
def fetch_paperlog_rows(conn: Any) -> list[PaperlogRow]:
    ...
    raw = conn.execute(
        f"SELECT log_date, ticker, source, decision_primary, "
        f"p_down_secondary, p_side_secondary, p_up_secondary, entry_close "
...
def _print_summary(stats: BackfillStats, planned: list[PlannedRow], committed: bool) -> None:
    mode = "COMMITTED" if committed else "DRY-RUN (no writes â€” pass --commit)"
...
def run(db_path: str, *, commit: bool, today: date | None = None) -> BackfillStats:
    """Orchestration: fetch â†’ plan â†’ summarize â†’ (optionally) insert."""
...
def main() -> None:
    ...
    parser.add_argument("--commit", action="store_true",
                        help="Write INSERTs (else dry-run â€” the default).")
