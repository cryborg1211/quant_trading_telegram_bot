...
$log = "logs\mr_breadth_analysis_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"

Add-Content $log "===== MR breadth-inflection analysis (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_mr_breadth_inflection.py >> `"$log`" 2>&1"
