...
$log = "logs\concentration_ab_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"

Add-Content $log "===== Concentration A/B (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_concentration_ab.py >> `"$log`" 2>&1"
