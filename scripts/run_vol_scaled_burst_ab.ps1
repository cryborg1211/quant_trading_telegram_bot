...
$log = "logs\vol_scaled_burst_ab_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"

Add-Content $log "===== Vol-scaled burst A/B (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_vol_scaled_burst_ab.py >> `"$log`" 2>&1"
