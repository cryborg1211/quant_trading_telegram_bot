...
$log = "logs\ssi_foreign_flow_backfill_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"

Add-Content $log "===== SSI FastConnect full-universe foreign-flow backfill : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/backfill_ssi_foreign_flow.py >> `"$log`" 2>&1"
