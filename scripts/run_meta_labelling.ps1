...
$log = "logs\meta_labelling_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
Add-Content $log "===== Meta-labelling (read-only) : $(Get-Date -Format o) ====="
cmd /c "`"$py`" scripts/analyze_meta_labelling.py models/saved/t20_std_checkpoint.joblib >> `"$log`" 2>&1"
