...
**GOLDEN selection now carries a drawdown budget (`41390d6`).** It was max mean OOS Net PnL with no risk term, which is unsafe here: Sharpe carries no signal while mean DD is monotone in the gate, so maximising PnL **systematically walks toward maximum drawdown** — and the arms are not even separable on PnL (0.42 sits 1.8% behind 0.43, inside seed noise), so a different seed draw picks −14.83% or −16.20% on an unattended run. `run_backtest._select_golden` now takes max mean NetPnL **subject to** `CONFIG.trading.golden_max_mean_dd_pp` (**14.0**, CLI `--golden-max-mean-dd-pp`, `0` restores plain max-PnL). 14.0 caps the tail without relitigating a reviewed run: it admits the 0.43 arm the dry sweep picked (−13.88%) and blocks 0.42/0.41. Budgeted on **mean** DD across seeds, not the best seed's (the artifact stores the best seed's −12.54%; the mean is the expected case). Sign-agnostic. If NO arm fits it returns the **shallowest-DD** arm rather than aborting — a retrain must still produce a candidate. Logs `GOLDEN drawdown budget BINDING` at WARNING when PnL wanted a deeper arm. Deliberately NOT the promote-gate's job: `promote_gate_max_abs_dd_pp` stays loose at 25% so it only blocks runaways; tightening it to the band would re-create the deadlock just repaired. Tests: `tests/test_golden_dd_budget.py` (11).
